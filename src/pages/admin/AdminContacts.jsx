...
import { adminGetContacts, adminUpdateContactStatus, adminDeleteContact } from "../../services/api";

const STATUS_COLORS = { pending: "#f0a500", read: "#4dabf7", responded: "#3aafa9" };
...
export default function AdminContacts() {
  ...
  const load = () => {
    setLoading(true);
    adminGetContacts({ page, limit, ...(filter ? { status: filter } : {}) })
      .then(res => { setContacts(res.data || []); setTotal(res.pagination?.total || 0); })
  ...
  const s = {
    ...
    fieldLabel: { fontSize: "12px", color: "#888", marginBottom: "4px" },
    fieldValue: { fontSize: "14px", color: "#333", marginBottom: "16px" },
  };
      ...
      <div style={s.card}>
        {loading ? <p style={{ textAlign: "center", padding: "40px", color: "#888" }}>Loading...</p> : (
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {contacts.length === 0 && (
                <tr><td colSpan={7} style={{ ...s.td, textAlign: "center", color: "#aaa" }}>No contacts found.</td></tr>
              )}
              {contacts.map((c, i) => (
                <tr key={c.id} style={{ cursor: "pointer" }} onClick={() => setSelected(c)}>
                  ...
                  <td style={s.td} onClick={e => e.stopPropagation()}>
                    <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                      <button style={s.actionBtn("#4dabf7")} onClick={() => updateStatus(c.id, "read")}>Mark Read</button>
                      <button style={s.actionBtn("#3aafa9")} onClick={() => updateStatus(c.id, "responded")}>Responded</button>
                      <button style={s.actionBtn("#2a9d8f")} onClick={() => handleDelete(c.id)}>Delete</button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div style={s.modal} onClick={() => setSelected(null)}>
          <div style={s.modalBox} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
              <h2 style={{ fontSize: "18px", fontWeight: 700, color: "#1b3d4f" }}>Contact Detail</h2>
              <button onClick={() => setSelected(null)} style={{ background: "none", border: "none", fontSize: "22px", cursor: "pointer", color: "#888" }}>x</button>
            </div>
            <p style={s.fieldLabel}>Name</p><p style={s.fieldValue}>{selected.name}</p>
            <p style={s.fieldLabel}>Email</p><p style={s.fieldValue}>{selected.email}</p>
            <p style={s.fieldLabel}>Phone</p><p style={s.fieldValue}>{selected.phone}</p>
            {selected.dob && <><p style={s.fieldLabel}>Date of Birth</p><p style={s.fieldValue}>{selected.dob}</p></>}
            <p style={s.fieldLabel}>Message</p>
            <p style={{ ...s.fieldValue, background: "#f8f9fa", padding: "12px", borderRadius: "8px", lineHeight: 1.6 }}>{selected.message}</p>
            <p style={s.fieldLabel}>Update Status</p>
            <div style={{ display: "flex", gap: "8px", marginBottom: "20px" }}>
              {["pending", "read", "responded"].map(st => (
                <button key={st} style={{ ...s.actionBtn(STATUS_COLORS[st]), padding: "8px 16px", outline: selected.status === st ? "2px solid " + STATUS_COLORS[st] : "none" }}
                  onClick={() => updateStatus(selected.id, st)}>{st}</button>
              ))}
            </div>
            <button style={{ width: "100%", padding: "10px", background: "#2a9d8f", color: "#fff", border: "none", borderRadius: "8px", fontWeight: 700, cursor: "pointer" }}
              onClick={() => handleDelete(selected.id)}>Delete Message</button>
          </div>
        </div>
      )}
    </AdminLayout>
